...
class ShiftPlanActionHandler:
    ...
    def _trigger_targeted_update(self, user_id, date_obj, old_shift, new_shift):
        ...
        try:
            ...
            dm.recalculate_daily_counts_for_day(date_obj, old_shift, new_shift)
             ...
             messagebox.showwarning("Update-Fehler", f"Interne Daten konnten nicht vollständig aktualisiert werden:\n{e}\n\nUI wird möglicherweise inkonsistent.", parent=self.tab)
        ...
        if self.renderer:
            try:
                ...
                if day > 0:
                 ...
                 messagebox.showerror("UI Update Fehler", f"UI konnte nicht gezielt aktualisiert werden:\n{e}\n\nBitte manuell neu laden (Monat wechseln).", parent=self.tab)
        else:
             print("[FEHLER] Renderer nicht verfügbar für UI-Updates.")
             messagebox.showerror("Interner Fehler", "Renderer-Komponente nicht gefunden. UI kann nicht aktualisiert werden.", parent=self.tab)
    ...
    def save_shift_entry_and_refresh(self, user_id, date_str, shift_abbrev):
        ...
        try:
            ...
            else: print("[WARNUNG] Renderer/Widgets nicht bereit für alten Wert.")
        except Exception as e:
        ...
        if success:
            ...
            if old_shift_abbrev and old_shift_abbrev in self.app.shift_frequency: self.app.shift_frequency[old_shift_abbrev] = max(0, self.app.shift_frequency[old_shift_abbrev] - 1)
            # Zähle neue Schicht nur, wenn sie gültig ist (nicht leer, U, X etc.)
            if actual_shift_to_save and actual_shift_to_save in self.app.shift_types_data and actual_shift_to_save not in ['U', 'X', 'EU']:
    ...
    def on_grid_cell_click(self, event, user_id, day, year, month):
        date_obj = date(year, month, day); date_str = date_obj.strftime('%Y-%m-%d')
        ...
        if not hasattr(self.tab, '_menu_item_cache') or not self.tab._menu_item_cache:
             ...
             if hasattr(self.tab, '_prepare_shift_menu_items'):
                 try: self.tab._menu_item_cache = self.tab._prepare_shift_menu_items()
                 except Exception as e: print(f"Cache-Fehler: {e}"); messagebox.showerror("Fehler", "Menü init failed.", parent=self.tab); return
             else: messagebox.showerror("Fehler", "Menü kann nicht erstellt werden.", parent=self.tab); return
        if hasattr(self.tab, '_menu_item_cache') and self.tab._menu_item_cache:
             for abbrev, label_text in self.tab._menu_item_cache: context_menu.add_command(label=label_text, command=lambda u=user_id, d=date_str, s=abbrev: self.save_shift_entry_and_refresh(u, d, s))
        else: context_menu.add_command(label="Fehler Schichtladen", state="disabled")
        context_menu.add_separator(); context_menu.add_command(label="FREI", command=lambda u=user_id, d=date_str: self.save_shift_entry_and_refresh(u, d, ""))
        context_menu.add_separator(); context_menu.add_command(label="Admin: Wunschfrei (WF)", command=lambda u=user_id, d=date_str: self.admin_add_wunschfrei(u, d, 'WF'))
        context_menu.add_command(label="Admin: Wunschfrei (T/N)", command=lambda u=user_id, d=date_str: self.admin_add_wunschfrei(u, d, 'T/N'))
        context_menu.tk_popup(event.x_root, event.y_root)

    # --- show_wunschfrei_context_menu (bleibt unverändert) ---
    def show_wunschfrei_context_menu(self, event, user_id, date_str):
        request = get_wunschfrei_request_by_user_and_date(user_id, date_str); context_menu = tk.Menu(self.tab, tearoff=0)
        if not request:
            print(f"Keine Anfrage für User {user_id} am {date_str}, zeige Admin-Optionen.")
            context_menu.add_command(label="Admin: Wunschfrei (WF)", command=lambda u=user_id, d=date_str: self.admin_add_wunschfrei(u, d, 'WF'))
            context_menu.add_command(label="Admin: Wunschfrei (T/N)", command=lambda u=user_id, d=date_str: self.admin_add_wunschfrei(u, d, 'T/N'))
        else:
            request_id = request['id']; status = request['status']; requested_shift = request['requested_shift']
            user_info = get_user_by_id(user_id); user_name = f"{user_info['vorname']} {user_info['name']}" if user_info else f"User ID {user_id}"
            if status == 'Ausstehend':
                context_menu.add_command(label=f"Anfrage von {user_name} ({requested_shift})", state="disabled"); context_menu.add_separator()
                if requested_shift == 'WF': context_menu.add_command(label="Akzeptieren (X setzen)", command=lambda rid=request_id, u=user_id, d=date_str: self.handle_request_accept_x(rid, u, d))
                elif requested_shift == 'T/N':
                    context_menu.add_command(label="Akzeptieren (T. setzen)", command=lambda rid=request_id, u=user_id, d=date_str: self.handle_request_accept_shift(rid, u, d, "T."))
                    context_menu.add_command(label="Akzeptieren (N. setzen)", command=lambda rid=request_id, u=user_id, d=date_str: self.handle_request_accept_shift(rid, u, d, "N."))
                else: context_menu.add_command(label=f"Akzeptieren ({requested_shift} setzen)", command=lambda rid=request_id, u=user_id, d=date_str, s=requested_shift: self.handle_request_accept_shift(rid, u, d, s))
                context_menu.add_command(label="Ablehnen", command=lambda rid=request_id, u=user_id, d=date_str: self.handle_request_reject(rid, u, d)); context_menu.add_separator()
                context_menu.add_command(label="Antrag löschen/zurückziehen", foreground="red", command=lambda rid=request_id, uid=user_id: self.handle_request_delete(rid, uid))
            else:
                 context_menu.add_command(label=f"Status: {status} ({requested_shift}) von {user_name}", state="disabled"); context_menu.add_separator()
                 context_menu.add_command(label="Zurücksetzen auf 'Ausstehend'", command=lambda rid=request_id: self.reset_request_status(rid))
                 context_menu.add_command(label="Antrag löschen/zurückziehen", foreground="red", command=lambda rid=request_id, uid=user_id: self.handle_request_delete(rid, uid))
        context_menu.tk_popup(event.x_root, event.y_root)
    ...
    def handle_request_accept_x(self, request_id, user_id, date_str):
        ...
        if success:
            ...
            self._update_dm_wunschfrei_cache(user_id, date_str, "Akzeptiert", "WF", 'user') # Annahme: User hat angefragt
            ...
            if save_success:
                ...
                except ValueError: messagebox.showerror("Fehler", "Datumsfehler.", parent=self.tab)
                if old_shift_abbrev and old_shift_abbrev in self.app.shift_frequency: self.app.shift_frequency[old_shift_abbrev] = max(0, self.app.shift_frequency[old_shift_abbrev] - 1)
                if "X" in self.app.shift_types_data: self.app.shift_frequency["X"] += 1
                self._refresh_requests_tab_if_loaded()
            else: messagebox.showerror("Fehler", f"Setzen auf 'X' fehlgeschlagen: {save_msg}", parent=self.tab)
        else: messagebox.showerror("Fehler", f"Status Update fehlgeschlagen: {msg}", parent=self.tab)
    ...
    def handle_request_accept_shift(self, request_id, user_id, date_str, shift_to_set):
        ...
        if success:
            ...
            if save_success:
                ...
                self._refresh_requests_tab_if_loaded()
            else: messagebox.showerror("Fehler", f"Setzen auf '{shift_to_set}' fehlgeschlagen: {save_msg}", parent=self.tab)
        else: messagebox.showerror("Fehler", f"Status Update fehlgeschlagen: {msg}", parent=self.tab)
    ...
    def handle_request_reject(self, request_id, user_id, date_str):
        ...
        dialog = RejectionReasonDialog(self.tab); reason = dialog.reason
        if reason is not None:
            ...
            if success:
                ...
                old_shift_abbrev = self._get_old_shift_from_ui(user_id, date_str)
                save_success, save_msg = save_shift_entry(user_id, date_str, "", keep_request_record=True) # Setze auf Frei
                if save_success:
                    try:
                        date_obj = datetime.strptime(date_str, '%Y-%m-%d').date()
                    ...
                    if old_shift_abbrev and old_shift_abbrev in self.app.shift_frequency: self.app.shift_frequency[old_shift_abbrev] = max(0, self.app.shift_frequency[old_shift_abbrev] - 1)
                    self._refresh_requests_tab_if_loaded()
                else: messagebox.showerror("Fehler", f"Status aktualisiert, Zelle leeren fehlgeschlagen: {save_msg}", parent=self.tab)
            else: messagebox.showerror("Fehler", f"Status Update fehlgeschlagen: {msg}", parent=self.tab)


    def handle_request_delete(self, request_id, user_id):
        """Löscht/Zieht einen Wunschfrei-Antrag zurück und aktualisiert UI gezielt."""
        if messagebox.askyesno("Löschen/Zurückziehen bestätigen", "Möchten Sie diesen Antrag wirklich löschen oder zurückziehen?", parent=self.tab):
            ...
            if success:
                ...
                if request_data and ("Akzeptiert" in request_data.get('status','') or "Genehmigt" in request_data.get('status','')):
                ...
                if date_str:
                    ...
                    except ValueError: print(f"[FEHLER] Ungültiges Datum (delete): {date_str}")
                    except Exception as e: print(f"Fehler bei UI Update nach Delete: {e}")
    ...
    def reset_request_status(self, request_id):
         ...
         if success:
              shift_that_was_removed = ""
              user_id = request_data['user_id']; date_str = request_data['request_date']
              ...
              if "Akzeptiert" in request_data.get('status','') or "Genehmigt" in request_data.get('status',''):
                   ...
                   if not save_success: messagebox.showwarning("Fehler", f"Status zurückgesetzt, Schicht entfernen fehlgeschlagen: {save_msg}", parent=self.tab)

              # Trigger UI Update
              try:
                  date_obj = datetime.strptime(date_str, '%Y-%m-%d').date()
                  self._trigger_targeted_update(user_id, date_obj, shift_that_was_removed, "") # Neuer Wert ""
              except ValueError: print(f"[FEHLER] Ungültiges Datum (reset): {date_str}")
              except Exception as e: print(f"Fehler bei UI Update nach Reset: {e}")
    ...
    def _refresh_requests_tab_if_loaded(self):
         ...
         if hasattr(self.app, 'refresh_specific_tab'):
             ...
             if hasattr(self.tab, 'master') and hasattr(self.tab.master, 'master') and hasattr(self.tab.master.master, 'loaded_tabs'):
    ...
    def admin_add_wunschfrei(self, user_id, date_str, request_type):
         ...
         if success:
              try:
                   ...
                   else: self.tab.refresh_plan() # Fallback
              except Exception as e:

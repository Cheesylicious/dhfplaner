...
class MainAdminWindow(tk.Toplevel):
    def __init__(self, master, user_data, app):
        ...
        # --- KORREKTUR: Binding erst nach initialem Laden ---
        # self.notebook.bind("<<NotebookTabChanged>>", self.on_tab_changed) # Wird jetzt nach dem ersten Laden gesetzt

        if self.notebook.tabs():
            # Initiales Laden des ersten Tabs (ohne Event-Auslösung)
            first_tab_name = list(self.tab_definitions.keys())[0]
            self._load_tab_directly(first_tab_name, 0) # Index ist 0

        # --- KORREKTUR: Binding jetzt erst setzen ---
        self.notebook.bind("<<NotebookTabChanged>>", self.on_tab_changed)

        print("[DEBUG] MainAdminWindow.__init__: Initialisierung abgeschlossen.")

    # --- Hilfsfunktion _finalize_tab_insert entfernt ---

    def _load_tab_directly(self, tab_name, tab_index):
        """Lädt einen Tab direkt, ohne auf Events zu reagieren. Wird für den initialen Tab genutzt."""
        if tab_name in self.loaded_tabs or tab_name in self.loading_tabs:
            return # Sollte beim Initialladen nicht passieren
        ...
        # Lade-Nachricht (optional für den ersten Tab, da es schnell gehen sollte)
        loading_label = ttk.Label(placeholder_frame, text=f"Lade {tab_name}...",
                                  font=("Segoe UI", 16), name="loading_label")
        ...
        try:
            # --- Tab-Lade-Logik (wie in on_tab_changed) ---
            if TabClass.__name__ == "UserTabSettingsTab": # Sollte hier nicht vorkommen
                ...
                try:
                    ...
                    print(f"[GUI - direct load] Warnung: {tab_name} mit (master, admin_window) fehlgeschlagen: {e1}. Versuche (master)...")
                    try:
            ...
            # --- Einfügen (DIREKT, ohne unbind/rebind, da kein Event gebunden ist) ---
            if placeholder_frame.winfo_exists():
                ...
                self.notebook.select(real_tab) # Wähle den ersten Tab aus
    ...
    def on_tab_changed(self, event):
        ...
        Lädt den Tab DIREKT IM GUI-THREAD mit unbind/rebind Schutz.
        """
        try:
            tab_id = self.notebook.select()
            if not tab_id: return
            # WICHTIG: Den Index *vor* jeglicher Änderung holen
            ...
            tab_name = tab_name_with_count.split(" (")[0]
             ...
             return # Frühzeitiger Ausstieg, wenn der Tab nicht ermittelt werden kann
        ...
        self.loading_tabs.add(tab_name) # Flag HIER setzen
        print(f"[GUI] on_tab_changed: Starte Ladevorgang für {tab_name} (im GUI-Thread)")

        TabClass = self.tab_definitions[tab_name]
        placeholder_frame = self.tab_frames.get(tab_name) # Sicherer Zugriff

        # Zusätzliche Prüfung: Existiert der Platzhalter überhaupt noch?
        ...
        # Alte Inhalte im Platzhalter entfernen
        ...
        try:
            ...
            if TabClass.__name__ == "UserTabSettingsTab":
                ...
                try:
                    ...
                    print(f"[GUI] Warnung: Konstruktor für {tab_name} mit (master, admin_window) fehlgeschlagen: {e1}. Versuche (master)...")
                    try:
                        real_tab = TabClass(self.notebook)
            ...
            if real_tab and tab_options:
                try:
                    # --- KRITISCHER BEREICH ---
                    self.notebook.unbind("<<NotebookTabChanged>>") # Event deaktivieren
                    self.notebook.forget(placeholder_frame)
                    placeholder_frame.destroy()
                    self.notebook.insert(tab_index, real_tab, **tab_options)
                    ...
                    # Event IMMER wieder aktivieren, auch wenn ein Fehler im kritischen Bereich auftritt
                    self.notebook.bind("<<NotebookTabChanged>>", self.on_tab_changed)
            ...
            if placeholder_frame.winfo_exists():
                for widget in placeholder_frame.winfo_children(): # "Lade..." entfernen
                    widget.destroy()
                ttk.Label(placeholder_frame, text=f"Fehler beim Laden:\n{e}",
                          font=("Segoe UI", 12), foreground="red").pack(expand=True, anchor="center")
            else:
            ...
            # Lade-Flag entfernen (wird jetzt nur noch einmal am Ende entfernt)
            if tab_name in self.loading_tabs:
                self.loading_tabs.remove(tab_name)
    ...
    def setup_styles(self):
        ...
        self.style.configure('Logout.TButton', background='gold', foreground='black', font=('Segoe UI', 10, 'bold'), padding=6)
    ...
    def update_single_tab_text(self, tab_name, new_text):
        # ... (unverändert, mit winfo_exists Check) ...
        frame = self.tab_frames.get(tab_name) # Sicherer Zugriff
        if frame:
            try:
                # Prüfen, ob der Frame noch ein gültiger Tab im Notebook ist
            ...
            except (tk.TclError, KeyError): # KeyError falls Widget-Pfad ungültig
    ...
    def switch_to_tab(self, tab_name):
        ...
        if frame and frame.winfo_exists():
            try:
                # Prüfen, ob der Frame noch ein Tab ist, bevor wir select aufrufen
    ...
    def _load_dynamic_tab(self, tab_name, TabClass, *args):
        """Lädt dynamische Tabs synchron im GUI-Thread mit unbind/rebind Schutz."""
        ...
        # --- Hinzufügen mit Unbind/Rebind ---
        tab_index = -1 # Initialisieren
        try:
            self.notebook.unbind("<<NotebookTabChanged>>")
            self.notebook.add(placeholder_frame, text=tab_name)
            self.notebook.select(placeholder_frame)
            tab_index = self.notebook.index(placeholder_frame) # Index nach dem Hinzufügen holen
            self.tab_frames[tab_name] = placeholder_frame
        finally:
             self.notebook.bind("<<NotebookTabChanged>>", self.on_tab_changed)
        ...
        try:
            ...
            if real_tab and tab_options and tab_index != -1: # Sicherstellen, dass Index gültig ist
                try:
                    # --- KRITISCHER BEREICH ---
                    self.notebook.unbind("<<NotebookTabChanged>>")
                    self.notebook.forget(placeholder_frame) # Vergessen vor Zerstören ist sicherer
                    placeholder_frame.destroy()
                    self.notebook.insert(tab_index, real_tab, **tab_options)
                    self.notebook.select(real_tab)
                    # --- ENDE KRITISCHER BEREICH ---
    ...
    def open_shift_types_window(self):
        ...
        try:
            ...
            try:
                from .tabs.shift_types_tab import ShiftTypesTab # Expliziter Import hier
    ...
    def setup_header(self):
        ...
        settings_menu.add_separator()
        settings_menu.add_command(label="⚠️ FIX: ALLE ALTEN USER FREISCHALTEN", command=self.apply_all_users_approval_fix)
    ...
    def apply_all_users_approval_fix(self):
        ...
        if messagebox.askyesno("ACHTUNG: FIX BESTÄTIGEN",
            ...
            if success: messagebox.showinfo("Erfolg: Login-Problem behoben", f"{message}\n\nBitte melden Sie sich nun ab und mit Ihren normalen Daten wieder an.", parent=self)
            else: messagebox.showerror("Fehler", message, parent=self)
    ...
    def apply_is_approved_fix(self):
        ...
        if messagebox.askyesno("Bestätigung",
            ...
            if success: messagebox.showinfo("Erfolg", message, parent=self)
            else: messagebox.showerror("Fehler", message, parent=self)
    ...
    def check_chat_notifications(self):
        ...
        if senders:
            ...
            notification_label = tk.Label(self.chat_notification_frame, text=label_text, bg='tomato', fg='white', font=('Segoe UI', 12, 'bold'), cursor="hand2")
    ...
    def go_to_chat(self, user_id):
        ...
        else: print("[DEBUG] go_to_chat: Chat-Tab ist noch nicht geladen oder 'select_user' fehlt.")
    ...
    def check_for_updates(self):
        ...
        if "Mitarbeiter" in self.loaded_tabs and hasattr(self.tab_frames["Mitarbeiter"], 'update_password_reset_button'):
            if self.tab_frames["Mitarbeiter"].winfo_exists():
    ...
    def update_header_notifications(self):
        ...
        if pending_password_resets > 0: notifications.append({"text": f"{pending_password_resets} Passwort-Reset(s)", "bg": "mediumpurple", "fg": "white", "action": self.open_password_resets_window})
        pending_wunsch_count = len(get_pending_wunschfrei_requests())
        if pending_wunsch_count > 0: notifications.append({"text": f"{pending_wunsch_count} Offene Wunschanfrage(n)", "bg": "orange", "fg": "black", "tab": "Wunschanfragen"})
        pending_urlaub_count = get_pending_vacation_requests_count()
        if pending_urlaub_count > 0: notifications.append({"text": f"{pending_urlaub_count} Offene Urlaubsanträge", "bg": "lightblue", "fg": "black", "tab": "Urlaubsanträge"})
        user_feedback_count = get_reports_with_user_feedback_count()
        if user_feedback_count > 0: notifications.append({"text": f"{user_feedback_count} User-Feedback(s)", "bg": "springgreen", "fg": "black", "tab": "Bug-Reports"})
        open_bug_count = get_open_bug_reports_count()
        if open_bug_count > 0: notifications.append({"text": f"{open_bug_count} Offene Bug-Report(s)", "bg": "tomato", "fg": "white", "tab": "Bug-Reports"})
        if not notifications:
            ttk.Label(self.notification_frame, text="Keine neuen Benachrichtigungen", font=('Segoe UI', 10, 'italic')).pack()
        else:
            for i, notif in enumerate(notifications):
                self.style.configure(f'Notif{i}.TButton', background=notif["bg"], foreground=notif["fg"])
                command = None
                if "action" in notif: command = notif["action"]
                else:
                    tab_name = notif.get("tab")
                    if tab_name: command = lambda tab=tab_name: self.switch_to_tab(tab)
                if command:
                    btn = ttk.Button(self.notification_frame, text=notif["text"], style=f'Notif{i}.TButton', command=command)
        ...
        if not rules:
            self.staffing_rules = {"Mo-Do": {}, "Fr": {}, "Sa-So": {}, "Holiday": {}, "Daily": {},
                                   "Colors": {"alert_bg": "#FF5555", "overstaffed_bg": "#FFFF99",
                                              "success_bg": "#90EE90", "weekend_bg": "#EAF4FF", "holiday_bg": "#FFD700"}}
        else: self.staffing_rules = rules
    ...
    def load_shift_frequency(self):
        # ... (unverändert) ...
        freq_data = load_shift_frequency(); return defaultdict(int, freq_data)

    def save_shift_frequency(self):
        # ... (unverändert) ...
        success = save_shift_frequency(self.shift_frequency)
        if not success: messagebox.showwarning("Speicherfehler", "Die Schichthäufigkeit konnte nicht in der Datenbank gespeichert werden.", parent=self)

    def reset_shift_frequency(self):
        # ... (unverändert) ...
        if messagebox.askyesno("Bestätigen", "Möchten Sie den Zähler für die Schichthäufigkeit wirklich zurücksetzen?", parent=self):
            success = reset_shift_frequency()
            if success:
                self.shift_frequency.clear(); messagebox.showinfo("Erfolg", "Der Zähler wurde zurückgesetzt.", parent=self)
            else: messagebox.showerror("Fehler", "Fehler beim Zurücksetzen in der Datenbank.", parent=self)
    ...
    def open_staffing_rules_window(self):
        ...
        def save_and_refresh(new_rules):
            if save_config_json(MIN_STAFFING_RULES_CONFIG_KEY, new_rules):
                self.staffing_rules = new_rules; self.refresh_shift_plan()
                messagebox.showinfo("Gespeichert", "Die Besetzungsregeln wurden erfolgreich aktualisiert.", parent=self)
            else: messagebox.showerror("Fehler", "Die Besetzungsregeln konnten nicht in der Datenbank gespeichert werden.", parent=self)
    ...
    def refresh_shift_plan(self):
        ...
        if "Schichtplan" in self.loaded_tabs:
            ...
            if frame and frame.winfo_exists():
                # Direkter Aufruf der refresh_plan Methode im Schichtplan-Tab
                frame.refresh_plan()

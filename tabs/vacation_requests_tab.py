...
class VacationRequestsTab(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        ttk.Button(action_frame, text="Genehmigung archivieren", command=self.archive_request).pack(side='left')
        ...
        ttk.Radiobutton(filter_frame, text="Archiv", variable=self.current_filter, value="Archiv",
                        command=self.load_requests).pack(side='left')

        # Treeview für die Anträge
        ...
        self.tree.tag_configure('Ausstehend', background='orange')
        self.tree.tag_configure('Archiviert', background='lightgrey')
    ...
    def load_requests(self):
        ...
        for req in all_requests:
            ...
            if filter_val == "Offen" and status != 'Ausstehend':
                continue
            if filter_val == "Archiv" and status != 'Archiviert':
                continue
            if filter_val == "Alle" and status == 'Archiviert':
                continue
    ...
    def archive_request(self):
        ...
        item = self.tree.item(request_id)
        if item['values'][3] != 'Genehmigt':
            messagebox.showinfo("Information", "Nur bereits genehmigte Anträge können archiviert werden.", parent=self)
            return

        if messagebox.askyesno("Bestätigen",
                               "Möchten Sie diesen genehmigten Urlaub wirklich archivieren?\nAlle 'EU' und 'X' Einträge für diesen Zeitraum werden entfernt.",
                               parent=self):

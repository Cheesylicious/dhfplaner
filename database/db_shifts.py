...
def get_consolidated_month_data(year, month):
    ...
    try:
        ...
        result_data = {'shifts': {}, 'daily_counts': {}, 'vacation_requests': [], 'wunschfrei_requests': {}, 'prev_month_shifts': {}}
        cursor.execute("SELECT user_id, shift_date, shift_abbrev FROM shift_schedule WHERE shift_date BETWEEN %s AND %s OR shift_date = %s", (start_date_str, end_date_str, prev_date_str))
        ...
        cursor.execute("SELECT ss.shift_date, ss.shift_abbrev, COUNT(ss.shift_abbrev) as count FROM shift_schedule ss LEFT JOIN user_order uo ON ss.user_id = uo.user_id WHERE ss.shift_date BETWEEN %s AND %s AND COALESCE (uo.is_visible, 1) = 1 GROUP BY ss.shift_date, ss.shift_abbrev", (start_date_str, end_date_str))
        for row in cursor.fetchall():
        ...
        cursor.execute("SELECT * FROM vacation_requests WHERE (start_date <= %s AND end_date >= %s) AND archived = 0", (end_date_str, start_date_str))
        result_data['vacation_requests'] = cursor.fetchall()
        cursor.execute("SELECT user_id, request_date, status, requested_shift, requested_by FROM wunschfrei_requests WHERE request_date BETWEEN %s AND %s", (start_date_str, end_date_str))
        for row in cursor.fetchall():
            user_id_str = str(row['user_id'])
            if user_id_str not in result_data['wunschfrei_requests']: result_data['wunschfrei_requests'][user_id_str] = {}
            result_data['wunschfrei_requests'][user_id_str][row['request_date']] = (row['status'], row['requested_shift'], row['requested_by'], None)
        return result_data
    except mysql.connector.Error as e: print(f"KRITISCHER FEHLER beim konsolidierten Abrufen der Daten: {e}"); return None
    finally:
...
def save_shift_entry(user_id, shift_date_str, shift_abbrev, keep_request_record=False):
    ...
    try:
        cursor = conn.cursor()
        if shift_abbrev in ["", "FREI"]: cursor.execute("DELETE FROM shift_schedule WHERE user_id = %s AND shift_date = %s", (user_id, shift_date_str))
        else: cursor.execute("INSERT INTO shift_schedule (user_id, shift_date, shift_abbrev) VALUES (%s, %s, %s) ON DUPLICATE KEY UPDATE shift_abbrev = %s", (user_id, shift_date_str, shift_abbrev, shift_abbrev))
        if shift_abbrev and shift_abbrev not in ["", "FREI"]:
            try:
                ...
                with open(events_file_path, 'r', encoding='utf-8') as f: all_events = json.load(f)
                ...
                if year_str in all_events and shift_date_str in all_events[year_str]:
                    ...
                    if event_type == "Quartals Ausbildung": cursor.execute("UPDATE users SET last_ausbildung = %s WHERE id = %s", (shift_date_str, user_id))
                    elif event_type == "Schießen": cursor.execute("UPDATE users SET last_schiessen = %s WHERE id = %s", (shift_date_str, user_id))
            except (FileNotFoundError, json.JSONDecodeError, KeyError, ImportError, AttributeError, NameError) as e: print(f"[WARNUNG] Konnte Event-Daten für Schichteintrag nicht prüfen: {e}"); pass
        if not keep_request_record and shift_abbrev != 'X': cursor.execute("DELETE FROM wunschfrei_requests WHERE user_id = %s AND request_date = %s", (user_id, shift_date_str))
        conn.commit()
        return True, "Schicht gespeichert."
    except mysql.connector.Error as e: conn.rollback(); return False, f"Datenbankfehler beim Speichern der Schicht: {e}"
    finally:
        if conn and conn.is_connected(): cursor.close(); conn.close()
...
def get_shifts_for_month(year, month):
    ...
    try:
        ...
        cursor.execute("SELECT user_id, shift_date, shift_abbrev FROM shift_schedule WHERE shift_date BETWEEN %s AND %s", (start_date, end_date))
        shifts = {}
        for row in cursor.fetchall():
        ...
        return shifts
    except mysql.connector.Error as e: print(f"Fehler beim Abrufen des Schichtplans: {e}"); return {}
    finally:
        if conn and conn.is_connected(): cursor.close(); conn.close()
...
def get_daily_shift_counts_for_month(year, month):
    ...
    try:
        ...
        start_date = date(year, month, 1).strftime('%Y-%m-01')
        end_date = date(year, month, calendar.monthrange(year, month)[1]).strftime('%Y-%m-%d')
        cursor.execute("SELECT ss.shift_date, ss.shift_abbrev, COUNT(ss.shift_abbrev) as count FROM shift_schedule ss LEFT JOIN user_order uo ON ss.user_id = uo.user_id WHERE ss.shift_date BETWEEN %s AND %s AND COALESCE(uo.is_visible, 1) = 1 GROUP BY ss.shift_date, ss.shift_abbrev", (start_date, end_date))
        daily_counts = {}
        for row in cursor.fetchall():
            shift_date = row['shift_date']
        ...
        return daily_counts
    except mysql.connector.Error as e: print(f"Fehler beim Abrufen der täglichen Schichtzählungen: {e}"); return {}
    finally:
        if conn and conn.is_connected(): cursor.close(); conn.close()
...
def get_all_shift_types():
    ...
    try:
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT id, name, abbreviation, hours, description, color, start_time, end_time, check_for_understaffing FROM shift_types ORDER BY abbreviation")
        results = cursor.fetchall()
        _SHIFT_TYPES_CACHE = results
        return results
    except mysql.connector.Error as e: print(f"Fehler beim Abrufen der Schichtarten: {e}"); _SHIFT_TYPES_CACHE = []; return []
    finally:
        if conn and conn.is_connected(): cursor.close(); conn.close()
...
def add_shift_type(data):
    ...
    try:
        ...
        params = (data['name'], data['abbreviation'], data['hours'], data.get('description', ''), data['color'], data.get('start_time') or None, data.get('end_time') or None, check_int)
        cursor.execute(query, params)
        conn.commit()
        clear_shift_types_cache(); clear_shift_order_cache()
        return True, "Schichtart erfolgreich hinzugefügt."
    except mysql.connector.IntegrityError: conn.rollback(); return False, "Eine Schichtart mit dieser Abkürzung existiert bereits."
    except mysql.connector.Error as e: conn.rollback(); return False, f"Datenbankfehler beim Hinzufügen: {e}"
    finally:
        if conn and conn.is_connected(): cursor.close(); conn.close()
...
def update_shift_type(shift_type_id, data):
    ...
    try:
        ...
        check_int = 1 if data.get('check_for_understaffing', False) else 0
        params = (data['name'], data['abbreviation'], data['hours'], data.get('description', ''), data['color'], data.get('start_time') or None, data.get('end_time') or None, check_int, shift_type_id)
        cursor.execute(query, params)
        conn.commit()
        clear_shift_types_cache(); clear_shift_order_cache()
        return True, "Schichtart erfolgreich aktualisiert."
    except mysql.connector.IntegrityError: conn.rollback(); return False, "Die neue Abkürzung wird bereits von einer anderen Schichtart verwendet."
    except mysql.connector.Error as e: conn.rollback(); return False, f"Datenbankfehler beim Aktualisieren: {e}"
    finally:
        if conn and conn.is_connected(): cursor.close(); conn.close()
...
def delete_shift_type(shift_type_id):
    ...
    except mysql.connector.Error as e: conn.rollback(); return False, f"Datenbankfehler beim Löschen: {e}"
    finally:
        if conn and conn.is_connected(): cursor.close(); conn.close()
        ...
        if order_data_list:
            ...
            for item_tuple in order_data_list:
                ...
                if len(item_tuple) == 3:
                     ...
                     print(f"[WARNUNG] Ungültiges Tupel in save_shift_order ignoriert (erwartet 3 Elemente): {item_tuple}")

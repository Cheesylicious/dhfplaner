    ...
    try:
        ...
        execute_create_table_if_not_exists("""
            CREATE TABLE IF NOT EXISTS `shift_types` (
              ...
              `check_for_understaffing` tinyint(1) DEFAULT '0',
              PRIMARY KEY (`id`),
        ...
        _add_column_if_not_exists(cursor, "users", "is_archived", "TINYINT(1) DEFAULT 0") # Nicht mehr AFTER is_approved
        _add_column_if_not_exists(cursor, "users", "archived_date", "DATETIME DEFAULT NULL") # Nicht mehr AFTER is_archived
...
def _log_activity(cursor, user_id, action_type, details):
    ...
    cursor.execute("INSERT INTO activity_log (timestamp, user_id, action_type, details) VALUES (%s, %s, %s, %s)", (timestamp, user_id, action_type, details))
...
def run_db_fix_approve_all_users():
    ...
    except mysql.connector.Error as e: conn.rollback(); return False, f"Ein Datenbankfehler ist aufgetreten: {e}"
    except Exception as e: conn.rollback(); return False, f"Ein unerwarteter Fehler ist aufgetreten: {e}"
    finally:
        if conn and conn.is_connected(): cursor.close(); conn.close()
...
def run_db_update_is_approved():
    ...
    except mysql.connector.Error as e: conn.rollback(); return False, f"Ein Fehler ist aufgetreten: {e}"
    finally:
        if conn and conn.is_connected(): cursor.close(); conn.close()

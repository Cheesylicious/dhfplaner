...
def get_all_vacation_requests_for_admin():
    ...
    try:
        ...
        cursor.execute("""
            ...
            ORDER BY 
                CASE 
                    WHEN vr.status = 'Ausstehend' THEN 1
                    WHEN vr.status = 'Genehmigt' THEN 2
                    WHEN vr.status = 'Abgelehnt' THEN 3
                    ELSE 4
                END, 
                vr.request_date ASC
        """)
...
def archive_vacation_request(request_id, admin_id):
    ...
    try:
        ...
        cursor.execute("UPDATE vacation_requests SET status = 'Archiviert', archived = 1 WHERE id = ?", (request_id,))
